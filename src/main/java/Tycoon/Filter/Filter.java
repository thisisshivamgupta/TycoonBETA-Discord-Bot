...
public class Filter extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        if(Tycoon.commands.FilterOnOff.filterOn) {
            String[] badWords = {"anal", "stfu", "mc", "madharchod", "lauda", "gandu", "fuck", "fucking", "pikina", "bsdk","bitch","bhadwa","chutka","jhat","jhaat","maachuda","gaandmarwa","fuckoff","fuck?","lowda","chutiya"};
                ...
                for (int j = 0; j < badWords.length; j++) {
                    if (message[i].equalsIgnoreCase(badWords[j])) {
                        ...
                        if(Tycoon.commands.FilterMessageOnOff.allowed== true) {
                            event.getChannel().sendMessage("Watch yo vernacular🧨").queue();
                        }
                    }
                }
            }
