...
public class Calculate extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
        ...
        if(message[0].equalsIgnoreCase(TycoonBETA.prefix +"cal") && message.length<3){
            e.getChannel().sendMessage("Follow the format.\n`~cal [add/sub] [First number: x] [Second number: y]`").queue();
        }
        else if(message[0].equalsIgnoreCase(TycoonBETA.prefix +"cal") && message[1].equalsIgnoreCase("add")){
       ...
       else if(message[0].equalsIgnoreCase(TycoonBETA.prefix +"cal") && message[1].equalsIgnoreCase("Sub")){
            int x= Integer.parseInt(message[2]);
            int y=Integer.parseInt(message[3]);
